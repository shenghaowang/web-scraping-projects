...
class Fundrazr(scrapy.Spider):
	...
	npages = 2
	# categories = ['Accidents', 'Alumni', 'Animals', 'Arts', 'Business', 'Celebrations', 
	# 			  'Community', 'Education', 'Faith', 'Family', 'Health', 'Legal', 'Memorials', 
	# 			  'Non-profits', 'Politics', 'Sports', 'Travel', 'Veterans']

	categories = ['Accidents', 'Business', 'Health']
	start_urls = []

	for category in categories:
		start_urls.append("https://fundrazr.com/find?category=" + category)
		for i in range(2, npages + 2):
			start_urls.append("https://fundrazr.com/find?category=" + category + "&page="+str(i)+"")
	...
	def parse(self, response):
		...
		start_position = parent_url.find('=')
		end_position = parent_url.find('&')
		category = parent_url[(start_position + 1):end_position]

		for href in response.xpath("//h2[contains(@class, 'title headline-font')]/a[contains(@class, 'campaign-link')]//@href"):
			# add the scheme, eg http://
			url  = "https:" + href.extract()
			new_callback = lambda response: self.parse_dir_contents(response, category)
			yield scrapy.Request(url, callback=new_callback)	
					
	def parse_dir_contents(self, response, category):
		item = FundrazrItem()
